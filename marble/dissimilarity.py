...
def absolute_dissimilarity(distribution, classes=None):
    ...
    unevenness) is defined as 

    ..math::
        D_{\alpha \beta} = \frac{1}{2 \left( 1 - \frac{N_\alpha}{N} \right)} \sum_{i=1}^{T} \left|
    \frac{n_\alpha(t)}{N_\alpha} - \frac{n(t)}{N} \right|

    where we compare the ratio `n_\alpha(t) / N_\alpha` to the expected value
    according to the null model. 

    Its value ranges from `0` (distribution undistinguishable from random
    configuration) to `1` (when all people of the class are gathered in a single
    tract).
    ...
    evenness: dictionary
        Classes dictionary with evenness as values.
        > {alpha: E_{\alpha}}
    """
    ## Regroup into classes if specified
    if classes is not None:
    ...
    evenness = {alpha: _single_evenness(distribution,
                                        alpha,
                                        N_unit,
                                        N_class,
                                        N_tot) 
